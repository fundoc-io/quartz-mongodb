...
public class MongoDBJobStore implements JobStore, Constants {
    ...
    private final MongoStoreAssembler assembler = new MongoStoreAssembler();
    ...
    /**
     * Configures the way job data is stored. {@link JobDetail}'s or {@link Trigger}'s {@link JobDataMap} can be
     * represented as a {@code Map<String,Object>}.
     *
     * <ul>
     * <li><b>{@code true}</b> (default) - Serialize map with {@link java.io.ObjectOutputStream ObjectOutputStream} and
     * store as {@code base64} encoded string in field '{@value Constants#JOB_DATA}'. Map may contain any
     * {@link java.io.Serializable Serializable} object internally, but will have some performance impact.
     * <li><b>{@code false}</b> - Store map directly in '{@value Constants#JOB_DATA_PLAIN}' field. Use this option is
     * you only store simple types in job data map for better performance.
     * </ul>
     */
     ...
     *             if an error occurred communicating with the MongoDB server.
     */
    private void ensureIndexes() throws SchedulerConfigException {
        try {
            /*
             * Indexes are to be declared as group then name. This is important as the quartz API allows for the
             * searching of jobs and triggers using a group matcher. To be able to use the compound index using group
             * alone (as the API allows), group must be the first key in that index.
             *
             * To be consistent, all such indexes are ensured in the order group then name. The previous indexes are
             * removed after we have "ensured" the new ones.
             */
