...
import io.fundoc.quartz.mongodb.cluster.*;
import io.fundoc.quartz.mongodb.dao.*;
import io.fundoc.quartz.mongodb.db.MongoConnector;
import io.fundoc.quartz.mongodb.db.MongoConnectorBuilder;
import io.fundoc.quartz.mongodb.trigger.MisfireHandler;
import io.fundoc.quartz.mongodb.trigger.TriggerConverter;
import io.fundoc.quartz.mongodb.util.Clock;
import io.fundoc.quartz.mongodb.util.ExpiryCalculator;
...
public class MongoStoreAssembler {
    ...
    public void build(MongoDBJobStore jobStore,ClassLoadHelper loadHelper,SchedulerSignaler signaler,
            ...
            throws SchedulerConfigException, ClassNotFoundException, IllegalAccessException, InstantiationException {
    ...
    private CheckinExecutor createCheckinExecutor(MongoDBJobStore jobStore,ClassLoadHelper loadHelper,
            Properties quartzProps) throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        return new CheckinExecutor(createCheckinTask(jobStore,loadHelper), jobStore.clusterCheckinIntervalMillis,
                jobStore.instanceId);
    }
    ...
    private Runnable createCheckinTask(MongoDBJobStore jobStore,ClassLoadHelper loadHelper)
        ...
        if (jobStore.getCheckInErrorHandler() == null) {
            // current default, see
            aClass = FailFastErrorHandler.class;
        } else {
    ...
    private LockManager createLockManager(MongoDBJobStore jobStore) {
        ExpiryCalculator expiryCalculator = new ExpiryCalculator(schedulerDao, Clock.SYSTEM_CLOCK,
                jobStore.jobTimeoutMillis, jobStore.triggerTimeoutMillis, jobStore.isClustered());
    ...
    private MongoConnector createMongoConnector(MongoDBJobStore jobStore) throws SchedulerConfigException {
        return MongoConnectorBuilder.builder().withConnector(jobStore.mongoConnector)
                .withDatabase(jobStore.mongoDatabase).withClient(jobStore.mongo).withUri(jobStore.mongoUri)
                ...
                .withTrustStore(jobStore.mongoOptionTrustStorePath,jobStore.mongoOptionTrustStorePassword,
                        jobStore.mongoOptionTrustStoreType)
                .withKeyStore(jobStore.mongoOptionKeyStorePath,jobStore.mongoOptionKeyStorePassword,
                        jobStore.mongoOptionKeyStoreType)
                .withWriteConcernWriteTimeout(jobStore.mongoOptionWriteConcernTimeoutMillis)
                .withWriteConcernW(jobStore.mongoOptionWriteConcernW).build();
    }
    ...
    private SchedulerDao createSchedulerDao(MongoDBJobStore jobStore) {
        return new SchedulerDao(getCollection(jobStore,"schedulers"), jobStore.schedulerName, jobStore.instanceId,
                jobStore.clusterCheckinIntervalMillis, Clock.SYSTEM_CLOCK);
    }
    ...
    private TriggerStateManager createTriggerStateManager() {
        return new TriggerStateManager(triggerDao, jobDao, pausedJobGroupsDao, pausedTriggerGroupsDao, queryHelper);
    }
