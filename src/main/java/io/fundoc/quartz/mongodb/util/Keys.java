...
public class Keys {
    ...
    public static Bson createTriggersLocksFilter(String instanceId) {
        return Filters.and(Filters.eq(Constants.LOCK_INSTANCE_ID,instanceId),Filters.eq(LOCK_TYPE,LockType.t.name()));
    }
    ...
    public static Document createLockUpdateDocument(String instanceId,Date newLockTime) {
        return new Document("$set",
                new Document().append(Constants.LOCK_INSTANCE_ID,instanceId).append(Constants.LOCK_TIME,newLockTime));
    }

    private static <T> Bson createLockFilter(LockType type,Key<T> key) {
        return Filters.and(Filters.eq(LOCK_TYPE,type.name()),Filters.eq(KEY_GROUP,key.getGroup()),
