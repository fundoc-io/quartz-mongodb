package io.fundoc.quartz.mongodb.trigger.properties;
...
import io.fundoc.quartz.mongodb.trigger.TriggerPropertiesConverter;
...
public class DailyTimeIntervalTriggerPropertiesConverter extends TriggerPropertiesConverter {
    ...
    public Document injectExtraPropertiesForInsert(OperableTrigger trigger,Document original) {
        ...
        return new Document(original).append(TRIGGER_REPEAT_INTERVAL_UNIT,t.getRepeatIntervalUnit().name())
    ...
    private Document toDocument(TimeOfDay tod) {
        return new Document().append("hour",tod.getHour()).append("minute",tod.getMinute()).append("second",
                tod.getSecond());
    }
