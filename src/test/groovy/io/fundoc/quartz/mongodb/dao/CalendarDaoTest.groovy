...
import static io.fundoc.quartz.mongodb.dao.CalendarDao.CALENDAR_NAME
import static io.fundoc.quartz.mongodb.dao.CalendarDao.CALENDAR_SERIALIZED_OBJECT
...
class CalendarDaoTest extends Specification {
	...
	def 'should return calendar names'() {
		...
		when:
		[nameFirst, nameSecond].forEach { name ->
			dao.store(
					name as String
